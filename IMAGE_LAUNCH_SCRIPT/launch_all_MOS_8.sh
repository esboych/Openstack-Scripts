...
node1=$(nova hypervisor-list | awk "/ 2 / "'{print $4}' | sed s/.lst.local//g)
#node1=$(nova hypervisor-list | awk "/ 1 / "'{print $4}')  # for Fue l5.1
echo "1st hypervisor:" $node1

node2=$(nova hypervisor-list | awk "/ 3 / "'{print $4}' | sed s/.lst.local//g)
...
echo "check are finished"
#image list
echo "images list:"
glance image-list
# Added 3rd instance launch
# Added floating IPs releasing back to pool


#new flawor
#nova flavor-create net.1-2Gb-32Gb auto 2048 32 1 --ephemeral 0 --swap 0  ## doesn;t work with this name
nova flavor-delete man-flavor
nova flavor-create man-flavor auto 2048 32 1 --ephemeral 0 --swap 0
...
#get net id ext04
#neutron_net_id=$(nova net-list | awk "/net04 / "'{print $2}') # MOS ver < 8.0
neutron_net_id=$(nova net-list | awk '/admin_floating/  {print $4}')  # MOS ver >= 8.0
...
then
    ...
    fixed_net_id=$(nova net-list | awk '/admin_internal_net/ {print $2'})
    float_net="admin_floating_net"
...
nova floating-ip-delete 172.16.53.78
nova floating-ip-delete 172.16.53.79
nova floating-ip-delete 172.16.53.80
nova floating-ip-delete 172.16.53.81
nova floating-ip-delete 172.16.53.82
nova floating-ip-delete 172.16.53.83
...
#launch 1st instance
nova boot --flavor man-flavor --image $trusty_image_id --key-name id_rsa-key --security-groups default --poll --nic net-id=$fixed_net_id  --availability-zone nova:$node1 Instance-nd1
#floating ip
f_ip=$(nova floating-ip-create "$float_net" | awk "/$float_net/"'{print $4}')
...
#launch 2nd instance
nova boot --flavor man-flavor --image $trusty_image_id --key-name id_rsa-key --security-groups default --poll --nic net-id=$fixed_net_id  --availability-zone nova:$node2 Instance-nd2
#floating ip
f_ip=$(nova floating-ip-create "$float_net" | awk "/$float_net/"'{print $4}')
