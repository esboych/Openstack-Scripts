...
#Command to connect to Rally DB with runs results
...
FROM (select task_name from test_timings_summary
		...
		OR jenkins_job like '11_env_run_rally/109/%'
    )) as main
LEFT OUTER JOIN
...
LEFT OUTER JOIN
    (select avg, jenkins_job, task_name from test_timings_summary where  jenkins_job like '11_env_run_rally/105/%') as four on four.task_name = main.task_name
LEFT OUTER JOIN
...
LEFT OUTER JOIN
    (select avg, jenkins_job, task_name from test_timings_summary where  jenkins_job like '11_env_run_rally/109/%') as six on six.task_name = main.task_name
ORDER BY main.task_name;\" "

#Go to Env11 Master node and run psql query on Rally DB
